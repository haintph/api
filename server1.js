...
class DeviceLicenseManager {
  ...
  static getClientIp(req) {
    ...
    if (!clientIp.ipv4 && !clientIp.ipv6) {
    ...
    // Trả về địa chỉ IP ưu tiên IPv4
    return clientIp.ipv4 || clientIp.ipv6 || '0.0.0.0';
  }
...
const verifyApiKey = async (req, res, next) => {
  ...
  // Kiểm tra nếu đang dùng máy ảo
  // if (networkInfo.virtualMachine) {
  //   return res.status(403).json({
  //     error: "Virtual machine detected",
  //     details: "Access from virtual environments is not allowed",
  //   });
  // }

  try {
    // Get API key data - MOVED THIS UP TO DEFINE keyData FIRST
    ...
    if (!allowedIP) {
      ...
      if (keyData.allowAutoRegister === 1) {
        ...
        // If no primary device fingerprint, set the current device as primary
        if (!keyData.primaryDeviceFingerprint) {
          await db.run(
        ...
        // Add the new IP with device identifier and details
        await db.run(
    ...
    const allowedIPs = await db.all(
      "SELECT ip, deviceIdentifier, deviceFingerprint FROM allowed_ips WHERE apikey_id = ?",
      [keyData.id]
    );
...
// Temporary key generation endpoint
...
apiRouter.get('/data-url', (req, res) => {
  ...
  res.json({ 
    ...
    deviceInfo: {
        ...
        browser: req.apiKeyData.currentDeviceData.browserName,
        deviceSimilarity: req.apiKeyData.deviceSimilarity,
        user: {
          windowsUsername: req.apiKeyData.currentDeviceData.windowsUsername,
          userHomedir: req.apiKeyData.currentDeviceData.userHomedir
        }
      },
